    ...
    public partial class Roster : Form
    {
        ...
        public void datagridview_rosterupdate_overall()
        {
            ...
            try
            {
                ...
                cmd.Parameters.Clear();
                cmd.CommandType = CommandType.Text;
                cmd.CommandText = "select top 50 RequestID,EmpName,ReportingManager,DateRoster,Day,Description,ProcessName,[Competency%],NoOfLeaves,[CoreTime%],Comments from dbo.vw_roster_dotnet where year(dateroster) = year(getdate()) order by DateRoster,EmpName asc";
                cmd.ExecuteNonQuery();
                sda.SelectCommand = cmd;
                sda.Fill(dt);
        ...
        public void datagridview_rosterupdate_filters()
        {
            ...
            try
            {
                ...
                {
                    cmd.CommandType = CommandType.Text;
                    cmd.CommandText = "select top 20 CurrentStatus as Description,EmpName,DateRoster,Day,[Competency%],ReportingManager,ProcessName,NoOfLeaves,RequestID,[CoreTime%],Comments from dbo.vw_roster_dotnet where dateadd(dd,1,eomonth(dateroster,-1)) = dateadd(dd,1,eomonth(@getdate,-1)) order by DateRoster,EmpName asc";
